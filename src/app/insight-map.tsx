...
type InsightTab = 'discover' | 'mindmap' | 'links';

export default function InsightMapScreen() {
  const {
    snapshot,
    cards,
    selectedNodeId,
  ...
  const [activeTab, setActiveTab] = useState<InsightTab>('mindmap');
    ...
    <>
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        ...
        <View style={styles.segmented}>
          ...
          <TabButton label="마인드맵" active={activeTab === 'mindmap'} onPress={() => setActiveTab('mindmap')} />
          <TabButton label="연결" active={activeTab === 'links'} onPress={() => setActiveTab('links')} />
        </View>
        ...
        {activeTab === 'discover' ? (
          <View style={styles.panel}>
            ...
            {cards.length === 0 ? (
              <Text style={styles.emptyText}>질문을 풀면 오늘의 발견 카드가 생겨요.</Text>
            ) : (
              cards.map((card) => (
                <Pressable
                  key={card.id}
                ...
                >
                  ...
                  <View style={styles.confidenceRow}>
                    <Text style={styles.confidenceLabel}>확신도</Text>
        ...
        {activeTab !== 'discover' ? (
          <View style={styles.panel}>
            <View style={styles.panelHeader}>
              <View>
                <Text style={styles.cardLabel}>{activeTab === 'links' ? '로컬 연결' : '성향 가지'}</Text>
            ...
            {snapshot ? (
              ...
              <View style={styles.loadingPanel}>
                ...
                <Text style={styles.emptyText}>성향 가지를 배치하는 중입니다.</Text>
              </View>
            )}
          </View>
        ) : null}
      ...
      <InsightNodeDetailSheet node={selectedNode} onClose={() => selectNode(null)} />
    </>
  );
}
...
const styles = StyleSheet.create({
  ...
  content: { padding: 20, paddingBottom: 46, paddingTop: 56 },
