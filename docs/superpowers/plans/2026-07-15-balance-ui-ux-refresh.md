...
- 지원 대상은 iOS, Android, Web이며 390×844 모바일 뷰를 기준으로 설계하고 Web은 최대 콘텐츠 폭 720px로 제한한다.
...
Expected: FAIL because `layout`, `typography`, and `spacing.touch` do not exist.
...
원격 URL은 `https:`만 허용하고 잘못된 URL, 빈 alt, 로딩 실패는 카테고리 fallback으로 전환한다. 오늘의 질문 seed에는 A/B 이미지 URL과 구체적인 alt를 모두 넣는다. UGC 생성 시에는 `{ imageUrl: null, alt: null }`을 저장한다.
...
test('announces the selected option and exact result after voting', async () => {
...
Hero는 badge → 질문 문구 → A/B media tiles → 결과 또는 안내 → pass 순서다. compact 카드는 category → 질문 → 선택 상태/요약 → 실제 참여 수 순서며 댓글·공감·저장 수는 렌더링하지 않는다. `ResultOverlay`는 흰 모달 overlay 대신 hero 내부 결과 panel로 바꾼다.
...
Expected: PASS; one physical press still produces at most one vote request.
...
**Interfaces:**
- Produces: `useDeckStore().goTo(index: number): void`
...
Expected: FAIL because compact cards and approved labels are absent.

- [ ] **Step 3: 피드와 탭 셸 구현**

`PlayScreen`의 쿼리·투표·오프라인 큐 로직은 유지하고 렌더링만 `ScrollView`의 daily hero와 compact list로 교체한다. compact 카드를 누르면 `goTo(index)` 후 해당 질문 hero로 스크롤한다. Web에서는 콘텐츠를 중앙 정렬하고 720px 이상 늘어나지 않게 한다.

- [ ] **Step 4: 피드·라우팅·Web export 실행**

Run: `cd mobile && npm test -- --runTestsByPath __tests__/play/play-screen.test.tsx __tests__/routing/tab-shell.test.tsx && npx expo export --platform web`

Expected: PASS and 12 static routes exported.
...
test('category chips expose one checked option and update selection', async () => {
  const view = await render(<CategoryChipGroup value="일상" options={['연애', '회사', '일상', '돈', '취향', '공부']} onChange={onChange} />);
...
화면 순서는 header → A/B 입력 section → category chips → 선택 설명 → 공개 범위/마감 setting rows → 게시하기다. 1차에서는 이미지 업로드를 제공하지 않는다. UGC 미디어는 null이며, 서버 실패 시 기존 입력과 선택된 카테고리·공개 범위를 그대로 유지한다.

- [ ] **Step 4: 작성 전체 회귀 테스트 실행**

Run: `cd mobile && npm test -- --runTestsByPath __tests__/ask/category-chip-group.test.tsx __tests__/ask/ask-screen.test.tsx __tests__/ask/create-question.test.ts`
...
Run: `cd mobile && npm test -- --runTestsByPath __tests__/brain/brain.test.ts __tests__/brain/brain-map.test.tsx __tests__/brain/brain-screen.test.tsx __tests__/analytics/analytics.test.ts`

Expected: PASS; evidence IDs never enter analytics payloads.
...
Expected: 291 pgTAP tests pass, DB lint returns zero findings, all Jest suites pass, Expo Doctor reports 20/20, and both exports succeed.
...
- large text: 200%
- Web narrow: 390×844
- Web desktop: 1280×900 with 720px centered content

각 크기에서 피드 첫 질문, 투표 후 결과, compact list, 작성 입력, 뇌 지도, 상세 공유를 캡처한다. clipping, horizontal scroll, 가려진 CTA, 겹친 bottom tab이 없어야 한다.
...
- 오늘의 이미지 필수와 일반 질문 이미지 선택 정책을 Task 2와 Task 3에 반영했다.
