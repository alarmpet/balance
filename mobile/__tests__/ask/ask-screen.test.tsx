...
test('creates link-only questions through the unified form', async () => {
  ...
  expect(view.queryByLabelText('질문 설명')).toBeNull();
  ...
  await fireEvent.press(view.getByText('추가 설정'));
  await fireEvent.press(view.getByText('링크로만 공개'));
  await fireEvent.press(view.getByRole('button', { name: '질문 등록' }));
...
test('does not submit duplicate options', async () => {
  ...
  expect(view.getByRole('button', { name: '질문 등록' })).toBeDisabled();
});
...
test('exposes disclosure and radio state to assistive technology', async () => {
  const view = await render(
  ...
  expect(view.getByRole('button', { name: '추가 설정 닫기' })).toHaveProp('accessibilityState', {
    expanded: true,
  });
  ...
  expect(publicRadio).toHaveProp('accessibilityState', { checked: true });
  expect(linkRadio).toHaveProp('accessibilityState', { checked: false });

  await fireEvent.press(linkRadio);
  expect(publicRadio).toHaveProp('accessibilityState', { checked: false });
  expect(linkRadio).toHaveProp('accessibilityState', { checked: true });
});
